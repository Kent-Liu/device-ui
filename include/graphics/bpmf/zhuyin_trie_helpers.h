#pragma once
// Which dictionary a build carries is a flash budget decision, and the three
// files declare the same BPMF_* arrays, so nothing below changes with it:
//   ZHUYIN_SINGLE_CHAR   single characters plus the commonest two-character
//                        words, for nRF52840 internal flash
//   ZHUYIN_FULL_CORPUS   every reading the corpus has, for boards with a
//                        multi-megabyte app partition
//   (neither)            the top 85,000 entries by frequency
#if defined(ZHUYIN_SINGLE_CHAR)
#include "zhuyin_single_static.h"
#elif defined(ZHUYIN_FULL_CORPUS)
#include "zhuyin_trie_full_static.h"
#else
